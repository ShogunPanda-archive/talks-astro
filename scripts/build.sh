...
TALK=$(echo "$TALKS" | fzf -1 -e --prompt "Which talk you want to build? " --info=hidden --preview-window=hidden --layout=reverse --height $FZF_HEIGHT --color $FZF_COLORS --query "$1")

if [ $? = 0 ]; then
  set -x -e
  rm -rf dist/$TALK
  ln -sf talks/$TALK.md current.md
  slidev build --out dist/$TALK current.md
fi
